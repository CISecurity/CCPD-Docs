...
#### Upload Report API ####
This api aims to upload reports (ARF,XML) generated by Assessor V3 and V4 into the Dashboard. This api is currently implemented by the Assessor using POST_URL feature.
This api can also be called from a script (python, powershell etc...). So organization having restricted network zone can use their own, organization-approved script in order to upload reports into the Dashboard. 
...
Note: ```[MY_DASHBOARD_AUTHENTICATION_TOKEN]``` is the token generated from the Dashboard with the api user. For more details, please follow [Establish authentication with Assessor](#establishAuthWithAssessor) section.<br/>
**'Content-type'**: ```multipart/form-data``` <br/>
Note: In the below example of the Python script, content-type is automatically generated behind the scene, no need to specify it. 
- **POST Data Params:** <br/>
...
Here is a script to upload a single report into the Dashboard:

Assuming ```Hostname_CIS_Microsoft_Windows_10_Enterprise_Release_1803_Benchmark-20190805T135433Z-ARF.xml``` is the name of the report file generated by CIS-CAT Pro Asssessor and located in ```./reports``` directory.<br/>The generated token is ```eertaa2pg2h7vb3ms97kdjebakr22v15``` and the Dashboard URL is ```https://mydashboard/CCPD/api/reports/upload``` 
